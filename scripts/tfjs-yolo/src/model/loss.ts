...
function yolo_boxes(pred: tf.Tensor, anchors:  tf.Tensor, classes: number): tf.Tensor[] {
...
export function getLastIndex(arr: tf.Tensor, num: number) {
...
function broadcast_iou(box_1: tf.Tensor, box_2: tf.Tensor) {
...
export function lossWrap (anchors: tf.Tensor, classes: number, ignore_thresh=0.5) {
  return function yoloLoss(yTrue: tf.Tensor, yPred: tf.Tensor): any {
